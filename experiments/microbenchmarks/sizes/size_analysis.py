...
def create_test_pg(path_to_data, target_feature, trained_pipeline, batch_sizes, index_sizes):
    ...
    for l in index_sizes:
        ...
        ########### finds distinct paths in the training and test data ##########
            ...
            cur.execute(sql.SQL(""" 
                                    DROP INDEX IF EXISTS pgml.test_index CASCADE;
                                    DROP TABLE IF EXISTS pgml.test_table CASCADE; 
                                    CREATE TABLE pgml.test_table as 
            ...
            cur.execute(sql.SQL(""" DROP INDEX IF EXISTS pgml.test_index CASCADE;
                                    CREATE index test_index on pgml.kv_table_test using spgist(key);"""))

            for iteration in range(5):


                ## First: Make the index scan not appeleing to the query planner:
                cur.execute(sql.SQL(""" 
                                        
                                        SET cpu_index_tuple_cost = 0.5;
                ...
                scoring_function = sql.SQL("""case when k2.value is not null then k2.value else pgml.prefix_search(k1.key, 'kv_table_test') end as prediction
                                                from (select * from pgml.test_table limit {batch}) k1
                                                left join pgml.kv_table_test k2
                                                on k1.key = k2.key
                                            """).format(batch=sql.SQL(str(b)))
                ...
                for row in result:
                    results.append([b, l, iteration, result[0][0]])
                
                cur.execute(sql.SQL("""explain analyze select case when k2.value is not null then k2.value else pgml.prefix_search(k1.key, 'kv_table_test') end as prediction
                                            from (select * from pgml.test_table limit {batch}) k1
                                            left join pgml.kv_table_test k2
                                            on k1.key = k2.key """
                                    ).format(batch=sql.SQL(str(b)))
                ...
                results[-1].append(analyze_fetched)

                ####### Create index and make querying the index extremely cheap.

                cur.execute(sql.SQL(""" 
                                        SET cpu_index_tuple_cost = 0.00000000001;
                                        SET enable_seqscan = TRUE;
    ...
    results_df = pd.DataFrame(results, columns=['Batch Size', 'Number of Features in Index', 'Iteration', 'Prediction Latency [ms] qpo', 'Query Plan Or.', 'Prediction Latency [ms] mod', 'Query Plan mod.' ])
...
# pg_df = create_test_pg(path, 'trip_duration', trained_pipeline, [100000],[10])
pg_df = create_test_pg(path, 'trip_duration', trained_pipeline, [1, 10, 100, 1000, 10000, 100000],[6, 10])
data_folder = os.path.join(project_folder, 'experiments', 'output', 'test_data_size_df.csv')
