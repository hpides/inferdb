...
def nyc_rides_lr():
    ...
    test = preprocessing_experiment('nyc_rides', NYC_Featurizer('deep'), LinearRegression())
    ...
    for i in range(5):
        ...
        project_folder = Path(__file__).resolve().parents[4]
        pipeline_path = os.path.join(project_folder, 'objects', 'nyc_rides_linearregression_complex_pipeline.joblib')
        object_path = os.path.join(project_folder, 'objects', 'nyc_rides_linearregression_complex_experiment.joblib')
    ...
    save_path = os.path.join(exp_folder, 'experiments', 'output', test.name + '_' + 'linearregression')
