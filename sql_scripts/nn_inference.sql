...
FROM   test_mat m1, nn_matrix_adult m2
WHERE  m2.id = 0
AND    m1.ord = m2.row
...
from input_weights iw, nn_matrix_adult nn
...
from activation m1, nn_matrix_adult nn
...
from output_weights m1, nn_matrix_adult nn
where nn.id = 3
-- group by 1, 2
)

select avg(round(val))
from results